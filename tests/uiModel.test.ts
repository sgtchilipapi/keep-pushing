...
function buildCharacter(overrides: Partial<CharacterReadModel> = {}): CharacterReadModel {
  return {
...
describe('game ui model helpers', () => {
  it('resolves skill and passive ids to display names with fallback to raw ids', () => {
    expect(resolveSkillNames(['1001', '9999'])).toEqual(['Volt Strike', '9999']);
    expect(resolvePassiveNames(['2001', '9999'])).toEqual(['Eagle Eye', '9999']);
  });
  ...
  it('reports pending first sync when backlog exists before confirmation', () => {
