...
function zoneStateAfterBatch(
  ...
  const delta = payload.zoneProgressDelta.find(
...
function deriveExpDelta(
  ...
  for (const entry of payload.encounterHistogram) {
    ...
    assertCondition(
      Boolean(zoneRegistry),
      "ERR_UNKNOWN_ZONE",
      "histogram references unknown zone",
    );
    assertCondition(
...
export function buildSettlementValidationContext(
  ...
  return {
    ...
    characterRoot: {
      characterId: accountCharacterIdHex(
        args.envelope.characterRoot.characterId,
      ),
      authority: args.envelope.characterRoot.authority.toBase58(),
      characterCreationTs: Number(
        args.envelope.characterRoot.characterCreationTs,
      ),
    },
    characterStats: {
    ...
    characterWorldProgress: {
      highestUnlockedZoneId:
        args.envelope.characterWorldProgress.highestUnlockedZoneId,
      highestClearedZoneId:
        args.envelope.characterWorldProgress.highestClearedZoneId,
    },
    zoneStates,
    cursor: {
      lastCommittedEndNonce: Number(
        args.envelope.characterBatchCursor.lastCommittedEndNonce,
      ),
      lastCommittedStateHash: accountStateHashHex(
        args.envelope.characterBatchCursor.lastCommittedStateHash,
      ),
      lastCommittedBatchId: Number(
        args.envelope.characterBatchCursor.lastCommittedBatchId,
      ),
      lastCommittedBattleTs: Number(
        args.envelope.characterBatchCursor.lastCommittedBattleTs,
      ),
      lastCommittedSeasonId:
        args.envelope.characterBatchCursor.lastCommittedSeasonId,
    ...
    programConfig: {
      ...
      maxHistogramEntriesPerBatch:
        args.envelope.programConfig.maxHistogramEntriesPerBatch,
      trustedServerSigner:
        args.envelope.programConfig.trustedServerSigner.toBase58(),
    },
    seasonPolicy: {
...
export function dryRunApplyBattleSettlementBatchV1(
  ...
  return applyBattleSettlementBatchV1(
    payload,
    buildSettlementValidationContext(args),
  );
}
...
export function applyBattleSettlementBatchV1(
  ...
  assertCondition(
    characterRoot.authority === playerAuthority,
    "ERR_UNAUTHORIZED",
    "player is not character authority",
  );
  assertCondition(
    characterRoot.characterId === payload.characterId,
    "ERR_CHARACTER_MISMATCH",
    "character id mismatch",
  );

  assertCondition(
    !programConfig.settlementPaused,
  ...
  assertCondition(
    payload.encounterHistogram.length <=
  ...
  assertCondition(
    ...
    "ERR_SCHEMA_VERSION",
    "schema version must be canonical V2",
  );
  assertCondition(
  ...
  assertCondition(
    payload.batchId === cursor.lastCommittedBatchId + 1,
    "ERR_BATCH_ID_GAP",
    "batch id must be monotonic",
  );
  assertCondition(
    payload.endNonce >= payload.startNonce,
    "ERR_NONCE_ORDER",
    "end nonce must be >= start nonce",
  );
  assertCondition(
  ...
  for (const entry of payload.encounterHistogram) {
    assertCondition(
      entry.count > 0,
      "ERR_HISTOGRAM_ZERO_COUNT",
      "histogram entry count must be > 0",
    );

    const key = `${entry.zoneId}:${entry.enemyArchetypeId}`;
    assertCondition(
      !keySet.has(key),
      "ERR_HISTOGRAM_DUPLICATE",
      "duplicate zone/enemy pair in histogram",
    );
    keySet.add(key);
  ...
  for (const delta of payload.zoneProgressDelta) {
    ...
    assertCondition(
      !zoneDeltaMap.has(delta.zoneId),
      "ERR_DUPLICATE_ZONE_DELTA",
      "duplicate zone progress delta zone id",
    );
  ...
  const allowedBattles =
    Math.floor((intervalSeconds * THROUGHPUT_CAP_PER_MINUTE) / 60) + 1;
  assertCondition(
  ...
  for (const entry of payload.encounterHistogram) {
    const effectiveZoneState = zoneStateAfterBatch(
      entry.zoneId,
      zoneStates,
      payload,
    );
    ...
    assertCondition(
      Boolean(allowedEnemies),
      "ERR_ZONE_ENEMY_SET_MISSING",
      "zone enemy set is missing",
    );
    assertCondition(
    ...
    assertCondition(
      zoneRegistry.has(entry.zoneId),
      "ERR_UNKNOWN_ZONE",
      "zone registry entry missing",
    );
    assertCondition(
